  ...
  for (var itemref in spineNode.findAllElements('itemref')) {
    ...
    if (manifestItems.containsKey(idref)) {
      ...
      if (exists) found++; else {
        ...
        final posixHref = p.posix.normalize(p.posix.join(p.posix.dirname(opfPath), Uri.decodeComponent(manifestNode.findAllElements('item').firstWhere((e) => e.getAttribute('id') == idref).getAttribute('href')!)));
        print('Did POSIX work? ' + posixHref + ' -> ' + archiveFiles.containsKey(posixHref).toString());
      }
    }
  }
